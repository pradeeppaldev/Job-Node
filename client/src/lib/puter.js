  ...
  const setError = (msg) => {
    set({
      ...
      auth: {
        ...
        signIn: get().auth?.signIn || signIn,
        signOut: get().auth?.signOut || signOut,
        refreshUser: get().auth?.refreshUser || refreshUser,
        checkAuthStatus: get().auth?.checkAuthStatus || checkAuthStatus,
        getUser: () => null,
      },
    });
  };
